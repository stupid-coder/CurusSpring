...
public class QuotaServiceUtils {
    ...
    static public int listQuotas(CurusDriver driver, Long days,
        ...
        //    }
            ...
            if ( days < 0 )
                quotaList = driver.quotaDao.selectLastestQuota(account_id,patient_id,quota_id,days*-1);
            else
                quotaList = driver.quotaDao.selectByMeasureDateLastDays(account_id,patient_id,quota_id,days);

            if ( quotaList != null && quotaList.size() > 0 ) {
                response.put("value",new JSONArray());
                for ( Quota quota : quotaList ) {
                    JSONObject jb = JSONObject.parseObject(quota.getRecord());
                    jb.put("measure_date", TimeUtils.date2Long(quota.getMeasure_date()));
                    response.getJSONArray("value").add(jb);
                }
            }
        }
